...
$conn->query("CREATE TABLE IF NOT EXISTS admin_messages_reply (
    id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    message_id INT NOT NULL,
    admin_id INT NULL,
    reply_text TEXT NOT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_msg_user_reply FOREIGN KEY (message_id) REFERENCES admin_messages(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;");

if (isset($_GET['mark_read'])) {
    $mid = (int)$_GET['mark_read'];
    if ($mid > 0) {
        if ($stmt = $conn->prepare("UPDATE admin_messages SET is_read=1 WHERE id=?")) {
            $stmt->bind_param("i", $mid);
            $stmt->execute();
            $stmt->close();
        }
    }
    header("Location: admin_inbox_user.php?view=".$mid);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['action'] ?? '')==='reply') {
    ...
    if ($mid<=0 || $text==='') {
        ...
        if ($ins = $conn->prepare("INSERT INTO admin_messages_reply (message_id, admin_id, reply_text) VALUES (?, ?, ?)")) {
            $ins->bind_param("iis", $mid, $admin_id, $text);
            if ($ins->execute()) {
                $ok = '✅ ส่งคำตอบเรียบร้อย';
                if ($u = $conn->prepare("UPDATE admin_messages SET is_read=1 WHERE id=?")) { $u->bind_param("i", $mid); $u->execute(); $u->close(); }
...
$viewReplies = [];
if ($viewId>0) {
    if ($q = $conn->prepare("SELECT m.id, m.subject, m.message, m.is_read, m.created_at, u.first_name, u.last_name, u.username, u.user_id FROM admin_messages m JOIN users u ON u.user_id = m.user_id WHERE m.id=?")) {
        ...
        $q->close();
        if ($viewMsg) {
            if ($r = $conn->prepare("SELECT reply_text, created_at, admin_id FROM admin_messages_reply WHERE message_id=? ORDER BY created_at ASC")) {
...
$unread = 0;
if ($rs = $conn->query("SELECT COUNT(*) c FROM admin_messages WHERE is_read=0")) { $row = $rs->fetch_assoc(); $unread = (int)($row['c'] ?? 0); $rs->close(); }
$per = 12; $page = max(1,(int)($_GET['page'] ?? 1)); $off = ($page-1)*$per;
$total = 0; if ($rs = $conn->query("SELECT COUNT(*) c FROM admin_messages")) { $total = (int)($rs->fetch_assoc()['c'] ?? 0); $rs->close(); }
$pages = max(1,(int)ceil($total/$per));
$list = [];
if ($rs = $conn->query("SELECT m.id, m.subject, m.is_read, m.created_at, CONCAT(u.first_name,' ',u.last_name) AS name FROM admin_messages m JOIN users u ON u.user_id = m.user_id ORDER BY m.created_at DESC LIMIT $per OFFSET $off")) {
...
<title>กล่องข้อความลูกค้า - Cameraman Admin</title>
...
<div class="navbar">
  <div class="nav-inner">
    ...
    <div class="menu" id="topMenu">
      ...
      <a href="admin_inbox_user.php" class="active">ข้อความลูกค้า</a>
    ...
    <div class="section">
      ...
      <div class="card">
        ...
        <?php else: ?>
          ...
          <div class="small">จาก <?= h($viewMsg['first_name'].' '.$viewMsg['last_name']) ?> (<?= h($viewMsg['username']) ?>) • เวลา <?= h(date('d/m/Y H:i', strtotime($viewMsg['created_at']))) ?></div>
